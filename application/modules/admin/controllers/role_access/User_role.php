...
class User_role extends MY_Controller
{
	...
	private $var_uri_seg_3  			= 'user_role';
	...
	public function index()
	{
		...
		# Content & Template
		# -------------------------------------------------------------------	
		$page_data['page_title'] 	= 'User Role';
		$page_data['content'] 		= 'role_access/user_role/view';
		// var_dump($page_data);
		// exit();
	...
	public function add()
	{
		...
		$page_data['user_role_name'] 	= '';
		$page_data['action'] 		= 'save';

		# Content & Template
		# -------------------------------------------------------------------	
		$page_data['page_title'] = 'Tambah User Role';
		$page_data['content'] = 'role_access/user_role/form';
		$this->load->view('template/template', $page_data);
	}
	...
	public function edit()
	{
		...
		$datato['table'] 					= 'gurumaya.user_role';
		$datato['where'] 					= array(
			'gurumaya.user_role.user_role_id' => $id
		);
		...
		if ($query->num_rows()) {
			...
			$page_data['user_role_name']		= $row->user_role_name;
			$page_data['action'] 			= 'save';

			# Content & Template
			# -------------------------------------------------------------------	
			$page_data['page_title'] 		= 'Edit User Role';
			$page_data['content'] 			= 'role_access/user_role/form';
			$this->load->view('template/template', $page_data);
		} else {
	...
	function save()
	{
		...
		# Validation Form
		# -------------------------------------------------------------------
		$this->form_validation->set_rules('user_role_name', 'Nama User Role', 'required');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$page_data['user_role_name']    		= $this->input->post('user_role_name');
			$page_data['action'] 				= 'save';
			...
			if ($type_form == "add") {
				...
				$datato['table'] 				= 'user_role';
				// $datato['company_id']			= $this->input->post('company_id');
				$datato['user_role_name']		 	= $this->input->post('user_role_name');
				// $datato['user_role_created_by'] 	= $sess_user_id;
				$datato['user_role_created_date']	= date('Y-m-d H:i:s');
			...
			if ($type_form == "edit") {
				...
				// $datato['company_id']			= $this->input->post('company_id');
				$datato['user_role_name']		 	= $this->input->post('user_role_name');
				// $datato['user_role_modified_by'] 	= $sess_user_id;
				$datato['user_role_modified_date'] = date('Y-m-d H:i:s');
				$datato['field'] 				= 'user_role_id';
				$datato['id'] 					= $id;
	...
	public function export_excel()
	{
		...
		foreach($Q1->result() as $R1){
			$index = 0;
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->user_role_id);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->user_role_name);
			// $excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $this->admin_model->get_user_fullname($R1->user_role_created_by));
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->user_role_created_date);		
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->user_role_modified_date);
			$excel_row++;
		}
		
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="report-data_user_role'.date('Y-m-d').'.xlsx"');
