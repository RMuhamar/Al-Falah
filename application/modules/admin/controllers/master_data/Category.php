...
class Category extends MY_Controller
{
	...
	private $var_uri_seg_3  			= 'category';
	...
	public function index()
	{
		...
		$page_data['content'] 		= 'master_data/category/view';
	...
	public function add()
	{
		...
		# Content & Template
		# -------------------------------------------------------------------	
		$page_data['page_title'] 		= 'Tambah Kategori';
		$page_data['content'] 			= 'master_data/category/form';
		$this->load->view('template/template', $page_data);
	}
	...
	public function edit()
	{
		...
		$datato['table'] 					= 'gurumaya.category';
		$datato['where'] 					= array(
			'gurumaya.category.category_id' => $id
		);
		...
		if ($query->num_rows()) {
			...
			$page_data['category_name']		= $row->category_name;
			$page_data['action'] 			= 'save';

			# Content & Template
			# -------------------------------------------------------------------	
			$page_data['page_title'] 		= 'Edit Kategori';
			$page_data['content'] 			= 'master_data/category/form';
			$this->load->view('template/template', $page_data);
		} else {
	...
	function save()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($type_form == "add") {
				...
				$datato['table'] 					= 'category';
				$datato['category_name']		 	= $this->input->post('category_name');
				$datato['category_created_by'] 		= $sess_user_id;
				$datato['category_created_date']	= date('Y-m-d H:i:s');
			...
			if ($type_form == "edit") {
				...
				$datato['table'] 					= 'category';
				$datato['category_name']		 	= $this->input->post('category_name');
				$datato['category_modified_by'] 	= $sess_user_id;
				$datato['category_modified_date'] = date('Y-m-d H:i:s');
	...
	public function export_excel()
	{
		...
		foreach($Q1->result() as $R1){
			...
			if($R1->category_active=1)
			{
				$category_active = 'Ya';
			}else if ($R1->category_active=0) {
				$category_active = 'Tidak';
			}
			
			$index = 0;
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->category_id);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->category_name);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $this->admin_model->get_user_fullname($R1->category_created_by));
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->category_created_date);
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $this->admin_model->get_user_fullname($R1->category_modified_by));
			$excel->getActiveSheet()->setCellValueByColumnAndRow($index++, $excel_row, $R1->category_modified_date);
			
			$excel_row++;
		}
		
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="master_data-category'.date('Y-m-d H:i:s').'.xlsx"');
